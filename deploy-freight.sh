...
scp -i ~/.ssh/ICEsoft_Linux_Test_Key_Pair.pem freight.tar.gz ubuntu@$HOST:~/. || { exit 1; }
echo "...Unpacking Dist on server..."
ssh -i ~/.ssh/ICEsoft_Linux_Test_Key_Pair.pem ubuntu@$HOST "sudo tar -zxf /home/ubuntu/freight.tar.gz -C /usr/share/nginx/html/static/demos/freight"
